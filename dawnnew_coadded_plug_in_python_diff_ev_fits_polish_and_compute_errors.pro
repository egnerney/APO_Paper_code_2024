...
function apo_deviates_emission_table_allside_simult_foursix_to_seven_7lines_18_params_fixed_necbyratio_tec_pl_dawn,p,x=x,y=y,err=err
   COMMON block1,x_grid,x_step_size,yptsi_in_2dto1d,nel2dto1d,tec2dto1d,Te_fixed,nec_fixed_found,rhoRne_,rhoORne_,rhocne_,Bin_,Bout_,nec_pl,totchmix_for_apo,nspmix,ns2pmix,nopmix,lambdaa,rhoRnsp_
 ...
 ; E = A*Exp(-((rhoORne - rhocne)/Bout)^2d) + C*Exp(-((rhoORne - rhoRne)/Dout)^2d)
  ...
  for idx = 0, n_elements(x_grid) -1 do begin
   ...
   if (x_grid(idx) le rhoRne) then tec[idx] = A_Te*((x_grid[idx]/rhoRne )^(B_te1))
   if (x_grid(idx) gt rhoRne) then tec[idx] = A_Te*((x_grid[idx]/rhoRne )^(B_te2))
   ...
   if ( nec_funcform_test(idx)  lt 0.000001d ) then begin
     ...
     nec_funcform_test(idx)  = 0.0000011d
...
function apodeviates_dawn_fixed_ratio_4timesgridnumparams,p,x=x,y=y,err=err
  ...
  nec_tempi  =  (nsp_tempi  + 2d*ns2p_tempi  + nop_tempi )/ totchmix_for_apo 
  ;nsp_to_nec_tot = total(nsp_tempi,/double)/total(nec_tempi,/double)
  ;ns2p_to_nec_tot = total(ns2p_tempi,/double)/total(nec_tempi,/double)
  ; nop_to_nec_tot = total(nop_tempi,/double)/total(nec_tempi,/double)
  ;totchmix = total(nsp_tempi + 2d*nsp_tempi + nop_tempi)/total(nec_tempi)
...
pro dawnnew_coadded_plug_in_python_diff_ev_fits_polish_and_compute_errors
 ...
 ; COMMON block2,x_grid,x_step_size,yptsi_in_2dto1d,nel2dto1d,tec2dto1d,Te_fixed,nec_fixed_found,rhoRne,rhoORne,rhocne
  COMMON block1,x_grid,x_step_size,yptsi_in_2dto1d,nel2dto1d,tec2dto1d,Te_fixed,nec_fixed_found,rhoRne_,rhoORne_,rhocne_,Bin_,Bout_,nec_pl,totchmix_for_apo,nspmix,ns2pmix,nopmix,lambdaa,rhoRnsp_ ;print,min(x_grid),max(x_grid)
  ;help,x_grid
  ;stop
  ;help,x_grid
  ;help,DAWN_GRID
 ...
 ; stop
  ...
  ;for llll = 0, 5 do begin
    ...
    yy=reform(dawn_grid_temp,ndawn_grid);[dawn_grid(n_elements(x_grid)-1-i,3),dawn_grid(n_elements(x_grid)-1-i,6),dawn_grid(n_elements(x_grid)-1-i,7)]
    errerr=reform(err_dawn_grid_temp,ndawn_grid);[50d*err_dawn_grid(n_elements(x_grid)-1-i,3),err_dawn_grid(n_elements(x_grid)-1-i,6),err_dawn_grid(n_elements(x_grid)-1-i,7)]
...
nwindow = 7;
totsmooth = ts_smooth(totchmix_for_apo,nwindow)

totsmooth_og = totsmooth
p1=plot(x_grid,totchmix_for_apo)
p2=plot(x_grid,totsmooth,/overplot,color='orange')



totchmix_for_apo = totsmooth_og;totsmooth
...
p1=plot(x_grid,dawn_grid(*,7)/dawn_grid(*,3),title='Black is 6731/4069, Red is 6716/4069 Dawn',color='black',xtitle='D $\rho_c$ $(R_J)$', ytitle='Line Ratios')
p2=plot(x_grid,dawn_grid(*,6)/dawn_grid(*,3),color='red',/overplot)

p1=plot(x_grid,dawn_grid(*,7)/dawn_grid(*,6),title='Black is 6731/6716',color='black',xtitle='Dawn $\rho_c$ $(R_J)$', ytitle='Line Ratios')

print,max(dawn_grid(*,3)), x_grid(where(dawn_grid(*,3) eq max(dawn_grid(*,3))))

print,max(dawn_grid(*,7)/dawn_grid(*,6)), x_grid(where(dawn_grid(*,7)/dawn_grid(*,6) eq max(dawn_grid(*,7)/dawn_grid(*,6))))
print,'start 8 line where max Dawn: '
for ggg = 7, 0,-1 do begin
  print,max(dawn_grid(*,ggg)), x_grid(where(dawn_grid(*,ggg) eq max(dawn_grid(*,ggg))))
endfor

p1=plot(x_grid,dawn_grid(*,6)/dawn_grid(*,7),xtitle='Dusk Distance ($R_J$)',ytitle='Rayleighs',yrange=[0,1.5],title='black is $S^+$ 6716/6731 and Red is $O^+$ 3729/3726 ',color='black')
  p2=plot(x_grid,dawn_grid(*,2)/dawn_grid(*,1),color='red',/overplot)
...
;stop
   ...
   ;stop
    ...
    ;COMMON block1,x_grid,x_step_size,yptsi_in_2dto1d,nel2dto1d,tec2dto1d,Te_fixed,nec_fixed_found,rhoRne_,rhoORne_,rhocne_,Bin_,Bout_,nec_pl,totchmix_for_apo,nspmix,ns2pmix,nopmix,lambdaa 
    nec_pl = 1.5d;5.4d; -3 is less steep than -5.4 or -(3+|x|) is less steep than -(5.4+|x|)

    rhocne = 5.28d
    rhoRne = 5.94d;5.98d ;DAWN
    rhoORne = 6.18d ; was 6.06d before for DAWN
    A = 1960d
    Bin = 0.3d
    Bout = 0.3d
    C = 3430d
    ...
    K2 = G2*Exp(-((rhoORnop - rhocnop)/Hout2)^2d) + I*Exp(-((rhoORnop - rhoRnop)/Jout2)^2d)
...
;r_given = 0.01d*dindgen(601) + 4.0d

;tec_innn = interpol(tec_innn,r_given, x_grid)
 ;nec_innn = interpol(nec_innn,r_given, x_grid)
 ;nsp_innn = interpol(nsp_innn,r_given, x_grid)
 ;ns2p_innn = interpol(ns2p_innn,r_given, x_grid)
 ;nop_innn = interpol(nop_innn,r_given, x_grid)
;restore,filename='save_file_cm3_fit_only_7_lines_emiss_tables_Dusk_coadded_0.04res_5pointavg.sav',/verbose
;restore,filename='save_file_cm3_fit_only_7_lines_emiss_tables_DAWN_coadded_0.04res_5pointavg.sav',/verbose

;p1=plot(x_grid,Tec_innn,layout=[2,4,1])
;p2=plot(x_grid,nec_innn,layout=[2,4,2],/current)
;p3=plot(x_grid,nsp_innn,layout=[2,4,3],/current)
;p4=plot(x_grid,nspmix,layout=[2,4,4],/current)
;p3=plot(x_grid,ns2p_innn,layout=[2,4,5],/current)
;p4=plot(x_grid,ns2pmix,layout=[2,4,6],/current)
;p3=plot(x_grid,nop_innn,layout=[2,4,7],/current)
;p4=plot(x_grid,nopmix,layout=[2,4,8],/current)
;stop

;model_innn = given_number_LOS_dens_temp_for_each_layer_simulate_rayleighs_dawn_or_dusk_all_at_once_8APO_lines_fixed_proper_nintegration_now(tec_innn, nec_innn,nsp_innn,ns2p_innn,nop_innn, max(x_grid),min(x_grid), x_step_size,yptsi_in_2dto1d,nel2dto1d,tec2dto1d)



;print,'did it'

;p1=plot(x_grid,model_innn(*,7),color='brown')

;p2=plot(x_grid,model_innn(*,6),/overplot,color='red')


;write_csv,'dawn_emission_to_subtract_off_try_hard_edge.csv',model_innn
;stop
   
openr,1,'varpls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_outs_and_errors_dawn_funcform_tec3partpl_python_60x16.csv'
outin = dblarr(16,60) ;
readf,1,outin
close,1
tec = outin[0,*]
nec_funcform_test = outin[2,*]
nsp_funcform_test =outin[4,*]
ns2p_funcform_test = outin[6,*]
nop_funcform_test = outin[8,*]
...
tec_fixed_found = reform(tec)
nec_fixed_found = reform(nec_funcform_test)
nsp_fixed_found = reform(nsp_funcform_test)
ns2p_fixed_found = reform(ns2p_funcform_test)
nop_fixed_found = reform(nop_funcform_test)
...
;for jjjj=0, 4 do begin

;if (jjjj eq  0) then parinfo[*].value = [replicate(2000d,n_elements(nec_funcform_test)),replicate(0.06d*2000d,n_elements(nsp_funcform_test))] ;[nec_funcform_test,nsp_funcform_test];[p_output_prev(*,1),p_output_prev(*,2)]
;if (jjjj gt  0) then parinfo[*].value = p ;[nec_funcform_test,nsp_funcform_test];[p_output_prev(*,1),p_output_prev(*,2)]
;parinfo[*].value = [replicate(2000d,n_elements(nec_funcform_test)),replicate(0.06d*2000d,n_elements(nsp_funcform_test))]
;help,[float(nec_fixed_found),float(nsp_fixed_found)]

;ns2p_fixed_found(56:59)  = 0.0001d*nec_fixed_found(56:59)
...
parinfo[0:3*nx_grid - 1].limits[1]  = 2000.d;d
...
;totchmix_for_apo=float(totchmix_for_apo)
;y=float(y)
;err=float(err)
;x=float(x)
fa = {X:x, Y:y, ERR:err}
...
p=mpfit('apodeviates_dawn_fixed_ratio_4timesgridnumparams',functargs=fa,parinfo=parinfo,perror=perror,MAXITER=0);,ftol=1e-20,xtol=1e-20,gtol=1e-20);,MAXITER=0);,ftol=1e-40,xtol=1e-40,gtol=1e-40,MAXITER=0)
;print,p
;print,perror
...
;nspmix[54:59] = nspmix[53] ; shifted out 1 one more bin when allowed fit paramters better (smaller sp inner cold torus width/gaussian radial thickness)
;ns2pmix[54:59] = ns2pmix[53]
;nopmix[54:59] = nopmix[53]

;nsp_fixed_found =  nec_fixed_found*nspmix
;nop_fixed_found =  nec_fixed_found*nopmix
;ns2p_fixed_found =  nec_fixed_found*ns2pmix

;nec_fixed_found = (nsp_fixed_found  + 2d*ns2p_fixed_found  + nop_fixed_found)/(totchmix_for_apo)

nec_error = sqrt((nsp_error^2d) + 4d*(ns2p_error^2d) + (nop_error^2d)) / totchmix_for_apo 

nspmix_error = nspmix*Sqrt((nsp_error/nsp_fixed_found)^2d + (nec_error/nec_fixed_found)^2d)

ns2pmix_error = ns2pmix*Sqrt((ns2p_error/ns2p_fixed_found)^2d + (nec_error/nec_fixed_found)^2d)

nopmix_error = nopmix*Sqrt((nop_error/nop_fixed_found)^2d + (nec_error/nec_fixed_found)^2d)
...
leg22.save,'dawn_diffev_python_model_vs_obs.png',resolution=300
...
p8.save,'dawn_diffev_python_simult_param_errorplots.png',resolution=300


p999 = errorplot(x_grid,totchmix,totchmix_error,ytitle='$\Sigma Z_i n_i $ /$n_{ec}$',xtitle='Dawn $\rho_c$ ($R_J$)',SYM_INCREMENT=2,ERRORBAR_CAPSIZE=0.01,yrange=[0,1.5],name='Fit');,/ylog,yrange=[0.01,1.5] )
...
leg = LEGEND(TARGET=[ p999, p9999], POSITION=[6.5,1.4], $
  ...
  write_csv,'dawn_diffev_output_to_plot60x13.csv',output

save,tec_fixed_found,tec_error,nec_fixed_found,nec_error,nsp_fixed_found,nsp_error,ns2p_fixed_found,ns2p_error,nop_fixed_found,nop_error,x_grid,nspmix_error,ns2pmix_error,nopmix_error,totchmix_for_apo, FILENAME = 'dawn_diffev_python_simult_param_save_file_simult_errors_from_mpfit_no_polish.sav',/verbose
