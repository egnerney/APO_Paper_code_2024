...
def residuals_function(p, x, y, err, x_grid, x_step_size, yptsi_in_2dto1d, nel2dto1d, tec2dto1d, rhoRne_, rhoORne_, rhocne_, rhoRnsp_, rho4069peak, nec_pl, totchmix_for_apo):
    ...
    for idx in range(nx_grid):
        ...
        if (((ns2p_funcform_test[idx] / nec_funcform_test[idx]) > 0.20) and (x_grid[idx] <= 5.80)):
            flags2p[idx] = 1.
        if (((nsp_funcform_test[idx] / nec_funcform_test[idx]) < 0.55) and (x_grid[idx] <= 5.27)):
            flagsp[idx] = 1.
        if (((nsp_funcform_test[idx] / nec_funcform_test[idx]) > 0.7) and (x_grid[idx] <= 5.27)):
            flagsp[idx] = flagsp[idx] + 1.
...
if __name__ == '__main__':
    ...
    dawn_grid1 = np.loadtxt('dawn_grid_to_plot_60x8.csv', delimiter=',')
    dawn_grid = np.transpose(dawn_grid1)

    err_dawn_grid1 = np.loadtxt('err_dawn_grid_to_plot_60x8.csv', delimiter=',')
    ...
    out_fits1 = np.loadtxt('outs_and_errors_dawn_funcform_fit_60x16.csv', delimiter=',')
    ...
    xtoplot = x_grid[14:40]
    aptochmix_to_plot =  interp1d(np.array([x_grid[14],x_grid[39]]), np.array([totchmix_for_apo[14],totchmix_for_apo[39]]), kind='linear')(xtoplot)
    ...
    bounds = [(5.2, 5.4), (450., 800.), (0.15, 0.5), (0.1, 0.6), (100., 1000.), (0.05, 0.5), (0.05, 1.5), (100., 1000.), (0.05, 0.8), (0.05, 2.5), (100., 1000.), (100., 1000.), (0.05, 0.5), (0.05, 2.5), (1., 2.5), (0.01, 10.), (0.01, 10.), (2.5, 8.),(5.79, 6.05), (5.90, 6.05), (5.79, 6.05),(5.45, 5.85), (5.86, 6.15),(5.98,6.2),(3.,10.),(2.,8.),(0.95,8.)]
    ...
    np.savetxt('varpls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_outs_and_errors_dawn_funcform_tec3partpl_python_60x16.csv', output_data, delimiter=',', comments='')
    np.savetxt('varpls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_dawn_funcform_params_pold_tec3partpl_python_27vals.csv', pold, delimiter=',', comments='')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig('varpls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_bestfit_params_python_dawn_funcform_tec3partpl.png')
    ...
    np.savetxt('varpls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_model_dawn_funcform_tec3partpl_python_60x8.csv', model_spectra, delimiter=',', comments='')
    plt.clf()
    fig, axs = plt.subplots(8, 1, figsize=(10, 20), sharex=True)
    ...
    plt.ylim([0,400])
    plt.legend(loc='best')
    plt.savefig('varpls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_bestfit_emiss_for_python_dawn_funcform_tec3partpl.png')
    plt.show()
