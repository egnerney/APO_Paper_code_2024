...
pro Dusknew_coadded_plug_in_python_diff_ev_fits_polish_and_compute_errors_cubic_spline_regression
...
totchmix_for_apo = interpol(totchmix_for_apo,rho_ceq_for_totchmix,x_grid) ; for DUSK
...
openr,1,'dusk_apo_final_totchmixratio.csv'
...
p1=plot(x_grid,dawn_grid(*,7)/dawn_grid(*,3),title='Black is 6731/4069, Red is 6716/4069 Dawn',color='black',xtitle='Dawn $\rho_c$ $(R_J)$', ytitle='Line Ratios')
  p2=plot(x_grid,dawn_grid(*,6)/dawn_grid(*,3),color='red',/overplot)

p1=plot(x_grid,dawn_grid(*,7)/dawn_grid(*,6),title='Black is 6731/6716',color='black',xtitle='Dawn $\rho_c$ $(R_J)$', ytitle='Line Ratios',xrange=[5.5,5.8])

  print,max(dawn_grid(*,3)), x_grid(where(dawn_grid(*,3) eq max(dawn_grid(*,3))))

x_grid_temp = x_grid(20:40)
print,max(dawn_grid(20:40,7)/dawn_grid(20:40,6)), x_grid_temp(where(dawn_grid(20:40,7)/dawn_grid(20:40,6) eq max(dawn_grid(20:40,7)/dawn_grid(20:40,6))))
   print,'start 8 line where max Dusk: '
...
p1=plot(x_grid,dawn_grid(*,6)/dawn_grid(*,7),xtitle='Dusk Distance ($R_J$)',ytitle='Rayleighs',yrange=[0,1.5],title='black is $S^+$ 6716/6731 and Red is $O^+$ 3729/3726 ',color='black')
  p2=plot(x_grid,dawn_grid(*,2)/dawn_grid(*,1),color='red',/overplot)
    ...
    nec_pl = 3d;5.4d; -3 is less steep than -5.4 or -(3+|x|) is less steep than -(5.4+|x|)

    rhocne = 4.95d
    rhoRne = 5.65d;5.98d ;Dusk
    
    rhoRns2p_ =  rhoRne ; tried 5.75d before Dusk
    Din2max = 0.2d
    
    rhoORne = 5.82d ;Dusk tried 5.94 too thought og 5.82 better
    A = 1960d
    Bin = 0.3d
    Bout = 0.3d
    C = 3430d
...
;rhoRne5.65_rhoRnsp5.72
   ...
   Jin = 0.142d
    ...
    A_TE1 = 2.1d;5.d; 3.07d
   
    B_te1 = 6.76d;9.724d;7.96d
    B_te2 = 1.823d; 1.968d; 2.28d
...
openr,1,'varpls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_outs_and_errors_dusk_funcform_tec3partpl_python_60x16.csv'
outin = dblarr(16,60) ;
readf,1,outin
close,1
tec = reform(outin[0,*])
...
model = given_number_LOS_dens_temp_for_each_layer_simulate_rayleighs_dawn_or_dusk_all_at_once_8APO_lines_fixed_proper_nintegration_now(tec, nec_funcform_test,nsp_funcform_test,ns2p_funcform_test,nop_funcform_test, max(x_grid),min(x_grid), x_step_size,yptsi_in_2dto1d,nel2dto1d,tec2dto1d)
   ...
   ; lambda1_arr = [1d-3,1d-6,1d-12,1d-18,1d-24,1d-30,1d-36,10d] ; lambda1 is for p[0:59] = nec; lambda2 is for p[60:119] = Tec
    
    
    ;lambda1_arr = [1d-1,1d-2,5d-3,0.5d,1d,5d,10d] 
    
   ; lambda1_arr = [1d-5,0.2d-6,0.4d-6,0.6d-6,0.8d-6,1d-6,2d-7,4d-7,6d-7,8d-7,1d-7] ;[1d-6,1d-8,1d-10,1d-12,1d-7,1d-9,1d-11] 
    
   ; lambda2_arr = [2d-2,4d-2,6d-2,8d-2,1d-2,2d-3,4d-3,6d-3,8d-3,1d-3];[1d-4] ;[1d-1,5d-2,1d-2,5d-3,1d-3] 
    
    ;lambda1_arr = [1d-6,5d-7,1d-7];[1d-5,0.2d-6,0.4d-6,0.6d-6,0.8d-6,1d-6,2d-7,4d-7,6d-7,8d-7,1d-7] ;[1d-6,1d-8,1d-10,1d-12,1d-7,1d-9,1d-11]

   ;lambda2_arr = [1d-2,5d-3,1d-3];[2d-2,4d-2,6d-2,8d-2,1d-2,2d-3,4d-3,6d-3,8d-3,1d-3];[1d-4] ;[1d-1,5d-2,1d-2,5d-3,1d-3]


   lambda1_arr = [1d-7];[1d-5,0.2d-6,0.4d-6,0.6d-6,0.8d-6,1d-6,2d-7,4d-7,6d-7,8d-7,1d-7] ;[1d-6,1d-8,1d-10,1d-12,1d-7,1d-9,1d-11]

   lambda2_arr = [5d-3];[2d-2,4d-2,6d-2,8d-2,1d-2,2d-3,4d-3,6d-3,8d-3,1d-3];[1d-4] ;[1d-1,5d-2,1d-2,5d-3,1d-3]

   
   ;lambda1_arr  = [1d19]
    lambdafac = 5.08756d6 ; lambda2/lambda1 = lambda_Tec/Lambda_nec = lambdafac
    y_old = y
    err_old = err
    x_old = x
 ...
 lambdaa2 =   lambda2_arr[jjjjj];lambdaa1;*lambdafac 
   ...
   nec_fixed_found = p[0:nx_grid - 1]
   ;nec_error = perror[0:nx_grid - 1]

   tec_fixed_found = p[1*nx_grid:2*nx_grid - 1]
   ;tec_error = perror[1*nx_grid:2*nx_grid - 1]
...
leg22.save,'lambda1=' + trimmed_lambdaa1  + '_lambda2=' + trimmed_lambdaa2 + '_CSR_dusk_diffev_python_model_vs_obs.png',resolution=300
...
p8.save,'lambda1=' + trimmed_lambdaa1  + '_lambda2=' + trimmed_lambdaa2 + '_CSR_dusk_diffev_python_simult_param_errorplots.png',resolution=300
...
write_csv,'lambda1=' + trimmed_lambdaa1  + '_lambda2=' + trimmed_lambdaa2 + '_CSR_dusk_diffev_output_to_plot60x13.csv',output
write_csv,'lambda1=' + trimmed_lambdaa1  + '_lambda2=' + trimmed_lambdaa2 + '_CSR_dusk_diffev_model_to_plot60x8.csv',model



save,tec_fixed_found,tec_error,nec_fixed_found,nec_error,nsp_fixed_found,nsp_error,ns2p_fixed_found,ns2p_error,nop_fixed_found,nop_error,x_grid,nspmix_error,ns2pmix_error,nopmix_error,totchmix_for_apo,lambdaa1,lambdaa2, FILENAME = 'lambda1=' + trimmed_lambdaa1  + '_lambda2=' + trimmed_lambdaa2 + '_CSR_dusk_diffev_python_simult_param_save_file_simult_errors_from_mpfit_no_polish.sav',/verbose
;stop
 endfor
    endfor
