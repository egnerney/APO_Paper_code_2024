...
def residuals_function(p, x, y, err, x_grid, x_step_size, yptsi_in_2dto1d, nel2dto1d, tec2dto1d, rhoRne_, rhoORne_, rhocne_, rhoRnsp_, rho4069peak, nec_pl, totchmix_for_apo):
    ...
    for idx in range(nx_grid):
        ...
        if (((ns2p_funcform_test[idx] / nec_funcform_test[idx]) > 0.05) and (x_grid[idx] <= 5.3)):
            flags2p[idx] = 1.
        if (((ns2p_funcform_test[idx] / nec_funcform_test[idx]) > 0.183) and (x_grid[idx] <= 5.74)):
            flags2p[idx] = 1.
...
if __name__ == '__main__':
    ...
    spec_string = ['Dawn_night1_grid_to_plot_71x8.csv','Dawn_night2_grid_to_plot_71x8.csv','Dawn_night3_grid_to_plot_68x8.csv','Dawn_night4_grid_to_plot_67x8.csv','Dawn_night5_grid_to_plot_55x8.csv','Dawn_night6_grid_to_plot_64x8.csv']
    
    err_spec_string =  ['err_Dawn_night1_grid_to_plot_71x8.csv','err_Dawn_night2_grid_to_plot_71x8.csv','err_Dawn_night3_grid_to_plot_68x8.csv','err_Dawn_night4_grid_to_plot_67x8.csv','err_Dawn_night5_grid_to_plot_55x8.csv','err_Dawn_night6_grid_to_plot_64x8.csv']
    
    out_string = ['outs_and_errors_dawn_night1_funcform_fit_71x16.csv','outs_and_errors_dawn_night2_funcform_fit_71x16.csv','outs_and_errors_dawn_night3_funcform_fit_68x16.csv','outs_and_errors_dawn_night4_funcform_fit_67x16.csv','outs_and_errors_dawn_night5_funcform_fit_55x16.csv','outs_and_errors_dawn_night6_funcform_fit_64x16.csv']
   # idxs = [1,2,3,4,5]
    idxs = [1]
    for w in idxs:
       ...
       # err_dawn_grid = np.transpose(err_dawn_grid1)
        ...
        totchmix_for_apo = np.loadtxt('totchmix_for_apo_7pointavg_dawn_0.24_overall_shifted_from_dusk_unshfited.csv', delimiter=',')
        
       
        xtoplot = x_grid_coadded[14:40]
        aptochmix_to_plot =  interp1d(np.array([x_grid_coadded[14],x_grid_coadded[39]]), np.array([totchmix_for_apo[14],totchmix_for_apo[39]]), kind='linear')(xtoplot)
        totchmix_for_apo = np.concatenate([totchmix_for_apo[0:14],aptochmix_to_plot,totchmix_for_apo[40:]])
        ...
        #p0 = [5.06, 1150.,0.251,Hout,600.,Jin,Jout,650.,Din2,Dout2, 350., I2,Jin2,Jout2, A_TE1,B_te1 ,B_te2, A_TE2,5.66,5.7,5.7,5.58,5.82,5.82,6.37,4.13,3.96]
       # start_time = time.time()

       # bounds = [(4.80, 5.20), (400., 1450.), (0.1, 0.5), (0.1, 0.6), (100., 1000.), (0.05, 0.5), (0.05, 1.5), (100., 1000.), (0.05, 0.8), (0.05, 2.5), (100., 1700.), (100., 1700.), (0.05, 0.5), (0.05, 2.5), (1., 3.), (0.01, 10.), (0.01, 10.), (3., 8.),(5.55, 5.85), (5.55, 5.85), (5.55, 5.85),(5.25, 5.65), (5.66, 5.95),(5.75,6.05),(3.,10.),(2.,8.),(0.95,8.)]
        p0 = [5.28, 600.,0.251,Hout,600.,Jin,Jout,650.,Din2,Dout2, 350., I2,Jin2,Jout2, A_TE1,B_te1 ,B_te2, A_TE2,5.90,5.94,5.94,5.78,6.06,6.06,4.87, 2.63, 2.46]
        start_time = time.time()

        bounds = [(5.1, 5.45), (450., 1450.), (0.15, 0.5), (0.1, 0.6), (100., 1000.), (0.05, 0.5), (0.05, 1.5), (100., 1000.), (0.05, 0.8), (0.05, 2.5), (100., 1000.), (100., 1000.), (0.05, 0.5), (0.05, 2.5), (1., 2.5), (0.01, 10.), (0.01, 10.), (2.5, 8.),(5.70, 6.1), (5.70, 6.1), (5.70, 6.1),(5.45, 5.85), (5.86, 6.15),(5.95,6.3),(3.,10.),(2.,8.),(0.95,8.)]
        ...
        np.savetxt( nightname_strings[w] + '_varypls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_outs_and_errors_dawn_funcform_tec3partpl_python_60x16.csv', output_data, delimiter=',', comments='')
        np.savetxt( nightname_strings[w] + '_varypls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_dawn_funcform_params_pold_tec3partpl_python_23vals.csv', pold, delimiter=',', comments='')
        plt.clf()
        ...
        plt.tight_layout()
        plt.savefig( nightname_strings[w] + '_varypls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_bestfit_params_python_dawn_funcform_tec3partpl.png')
        ...
        np.savetxt( nightname_strings[w] + '_varypls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_model_dawn_funcform_tec3partpl_python_60x8.csv', model_spectra, delimiter=',', comments='')
        plt.clf()
        fig, axs = plt.subplots(8, 1, figsize=(10, 20), sharex=True)
        ...
        plt.xlabel(r'Dawn $\rho_c$ ($R_J$)')
        plt.ylabel('Rayleighs')
        plt.ylim([0,400])
        plt.legend(loc='best')
        plt.savefig( nightname_strings[w] + '_varypls_varytec_locs_varyrhoRnsp_and_rhoRns2p_and_nop_diff_ev_bestfit_emiss_for_python_dawn_funcform_tec3partpl.png')
        plt.show()
